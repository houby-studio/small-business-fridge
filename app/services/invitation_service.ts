...
export default class InvitationService {
  ...
  async createInvite(params: {
    ...
    return {
      invitation,
      inviteUrl: `${this.appUrl()}/register/invite/${rawToken}`,
    }
  }

  async validateToken(token: string): Promise<InviteTokenStatus> {
    const invitation = await UserInvitation.findBy('tokenHash', this.tokenHash(token))
  ...
  async acceptInvite(params: {
    ...
    return db.transaction(async (trx) => {
      const invite = await UserInvitation.query({ client: trx })
